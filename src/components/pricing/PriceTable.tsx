...
const PriceTable = () => {
      ...
      if (!ticking) {
        window.requestAnimationFrame(() => {
          ...
          if (
            ...
            (currentScrollY < 1600 && window.innerWidth < 650) ||
  ...
  return (
    <section className="bg-[#F6F6F8] px-4 pt-[20px] py-[70px] overflow-x-auto">
              ...
              <thead className="h-full">
                <tr className="h-full">
                  ...
                  {plans.map((plan, idx) => (
                    <th
                      key={plan.key}
                      className={`bg-[#F0F0F2] relative  w-1/4 align-top text-center ${
                        idx !== plans.length - 1
      ...
      {/* Sticky/absolute left arrow */}
      <button
        onClick={handlePrev}
        className={`fixed md:hidden left-[1px] sm:left-2 top-1/2 -translate-y-1/2 z-20 bg-white border border-[#24C16F]  p-1.5 shadow ${
      ...
      {/* Sticky/absolute right arrow */}
      <button
        onClick={handleNext}
        className={`fixed md:hidden right-[1px] sm:right-2 top-1/2 -translate-y-1/2 z-20 bg-white border border-[#24C16F]  p-1.5 shadow ${
          showArrows ? "opacity-100" : "opacity-0"
        }`}
